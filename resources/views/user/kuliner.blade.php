...
<main class="bg-gray-50 min-h-screen pt-8 pb-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 md:mb-8" data-aos="slide-up">
            <a href="{{ route('user.beranda.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-amber-900 hover:bg-amber-50 rounded-lg transition-colors duration-300" history.back()>
    ...
    <div data-aos="slide-up">
        ...
        <section class="mb-12 md:mb-16" data-aos="slide-up">
            <img src="{{ asset('storage/' . $data->gambar_kuliner) }}" 
                 alt="{{ $data->nama_kuliner }}" 
            ...
            <section data-aos="slide-up">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <div class="order-1 md:order-2">
                        <img src="{{ asset('storage/' . $data->gambar_kuliner) }}" 
                             alt="Fakta Unik {{ $data->nama_kuliner }}" 
                             class="w-full h-64 sm:h-80 md:h-96 object-cover rounded-lg shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-500 cursor-pointer">
                    </div>
                </div>
            </section>
                ...
                <div class="relative -mx-4 -my-8 md:-my-12 px-4 py-8 md:py-12">
                    ...
                    <div class="overflow-hidden">
                        <div id="carouselTrack" class="flex transition-transform duration-500 ease-out" style="transform: translateX(0)">
                            <!-- Spot Kuliner Card -->
                            @foreach ($data_all as $kuliner)
                            <div class="carousel-card w-full md:w-1/3 flex-shrink-0 px-4 py-9 md:px-5 md:py-12">
                                <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:scale-105 transition-all duration-500 h-full group cursor-pointer transform">
                                    ...
                                    <div class="p-4">
                                        <h3 class="text-lg font-bold text-gray-900 mb-2">{{ $kuliner->nama_kuliner }}</h3>
                                        <p class="text-sm text-gray-600 line-clamp-2 mb-3">
                                            {{ $kuliner->jenis_kuliner }}
                                        </p>
                                        <a href="{{ route('user.kuliner.show', ['id' => $kuliner->kuliner_id]) }}" class="text-amber-900 hover:text-amber-700 font-semibold text-sm flex items-center">
                                            Lihat Selengkapnya
